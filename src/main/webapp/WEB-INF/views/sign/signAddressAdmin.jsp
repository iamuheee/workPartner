...
<!-- css -->
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/resources/css/addressCss/address1.css">
<style>
.adContainer {
	display: grid;
	grid-gap: 20px;
	...
	grid-template-areas: 'header header  header header'
		' menu   main1  submit  main2' ' menu   page  submit  main2'
...
<script>
	function send() {
		let listCount = $("#myform tr").length;
		if( listCount > 3){
			alert("결재선은 3명까지 가능합니다.");
		}else{
			var data = $("#myform tr");
	...
	<div class="adOuter">
		...
		<div class="adContainer">
			...
			<div class="submit">
				...
				<button id="insertAddress" type="button"
					class="btn btn-sm btn-primary material-symbols-outlined"
					onclick="empNoSend();">arrow_forward_ios</button>
				...
				function empNoSend() {
					...
					if (count > 3 ) {
						...
						$("input[name='chk']:checked").each(
								function() {
									...
									const obj = {
										empNo : $(this).parent()
												.siblings(".no").text(),
										depCd : $(this).parent().siblings(
												".depCd").text(),
										empName : $(this).parent().siblings(
												".name").text()
									};
						...
						/* for(let i=0; i<arr.length; i++){
						   value += '<tr>'
							     +    '<th width="100px"> ' + 결재 + ' </th> ' 
						            +    '<td width="150px"> ' + arr[i].empName(arr[i].depCd) + ' </td> '
			...
			<div class="footer" align="end">
				...
				<input type="button" class="btn btn-sm btn-primary" value="보내기"
					onclick="send()" />
						...
						$.ajax({
							...
							success : function(result) {
								...
								if (list.length == 0) {
									...
									if (pi.currentPage != 1) {
										pageValue += "<button class='btn btn-sm btn-outline-primary' onclick='selectdepTbList("
												+ selectDepCd
												+ ", "
												+ (pi.currentPage - 1)
												+ ")'>&lt;</button>"
									...
									for (let p = pi.startPage; p <= pi.endPage; p++) {
										...
										if (p == pi.currentPage) {
											pageValue += "<button class='btn btn-sm btn-outline-primary' disabled>"
													+ p + "</button>"
										} else {
											pageValue += "<button class='btn btn-sm btn-outline-primary' onclick='selectdepTbList("
													+ selectDepCd
